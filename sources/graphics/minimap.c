...
/*   Updated: 2024/07/08 21:07:21 by gdelvign         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_calculate_cell_size(t_data *data, int *cell_size, int *offsets)
{
	...
	offsets[X] = (data->minimap->width - (cell_size[X] * data->map->x_size)) / 2;
	offsets[Y] = (data->minimap->height - (cell_size[Y] * data->map->y_size)) / 2;
}
	...
	while (i < data->map->y_size)
	{
		...
		while (j < data->map->x_size)
		{
			...
			while (++y < cell_size[Y])
			{
				x = 0;
				while (++x < cell_size[X])
					ft_img_pix_put(data->minimap->img, offsets[X] + j * cell_size[X] + x, offsets[Y] + i * cell_size[Y] + y, color);		
			}
			j++;
		}
		i++;
